...
public class AkkaMain {
    ...
    final Flow<HttpRequest, HttpResponse, NotUsed> actorFlow(Http http, ActorMaterializer actorMaterializer, ActorRef actorRef){
        return Flow.of(HttpRequest.class).map(h->{
            ...
            return new Pair<>(url, countFloat);
                ...
                .mapAsync(2, (Pair<String, Float> pair) ->
                    ...
                    if((float) o >= 0){
                        return CompletableFuture.completedFuture(new Pair<String, Float>(o.first(), (float)pair))
                    }
